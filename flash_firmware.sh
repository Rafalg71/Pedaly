...
echo "Wgrywanie pelnego firmware (bootloader + partitions + app) do $PORT..."
python3 -m esptool --chip esp32s3 --port $PORT --baud 460800 --before default_reset --after hard_reset write_flash -z 0x0 bin/bootloader.bin 0x8000 bin/partitions.bin 0x10000 bin/firmware.bin
